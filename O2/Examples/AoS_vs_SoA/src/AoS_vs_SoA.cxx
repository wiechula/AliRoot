...
double analyseMuons_flat(FlatTrack::flat_type *trackCollection, unsigned count,
  ...
  float arbitrary = 0;
  //Run the following for loop in parallel, while doing a reduction over arbitrary. Schedule splits the work into chunks of the given amount of iterations and dynamically allocates parts of it.
...
double analyseMuons(TrackAll &trackCollection, unsigned count,
  ...
  auto *z = trackCollection.Get<IPosition>().GetRef<ISpatial>().GetRef<Z>();
  float arbitrary = 0;
  //perform the following loop in parallel, reducing over arbitrary, splitting the work in chunks of 1024 iterations each
...
double analyseMuons_v(TrackAll &trackCollection, unsigned count,
  ...
  v8i cutoff = {RAND_MAX / oneIn, RAND_MAX / oneIn, RAND_MAX / oneIn,
                ...
                //get the arrays, reinterpret_cast to vectorized arrays. PoC for now, needs to be cleaner.
  v8i *muonScores =
...
#pragma omp parallel reduction(+ : arbitrary)
  {
    ...
    //parallel loop, count/8 because we are dealing with a vector of 8 elements each iteration
#pragma omp for schedule(dynamic, 128)
    for (unsigned u = 0; u < count / 8; u++) {
      v8i muon = muonScores[u];
      //what follows is a bit of magic to if 'muon < cutoff' for any of the 8 elements
      ...
      if (!_mm256_testz_si256(cmp, cmp)) { // checks if cmp is not all zero. i.e. at least one element of the 8 passes the filter

        v8f r = _mm256_sqrt_ps(x[u] * x[u] + y[u] * y[u] + z[u] * z[u]); //compute r for all 8 at a time
        v8f blend = _mm256_blendv_ps(zero, Pt[u] - r, (__m256)cmp);//pt-r if the corresponding element passes the filter, zero otherwise
