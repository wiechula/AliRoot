...
// the returned array can be `delete[]`ed. count is the number of tracks
// created.
FlatTrack::flat_type *make_flat(unsigned count);

// the following functions perform our fake "analysis", the select approximately
// one in "oneIn" tracks depeding on the value of the MuonScore track variable.
// If it passes the filter it will compute the length of the position vector and
// subtract this from the Pt. The resulting value is summed for all tracks that
// pass the filter.
// There are three version, the regular SoA version, the "_flat" version which
// uses SoA, and the "_vectorized" version which is SoA but uisng avx2 vector
// instructions.
double analyseMuons_flat(FlatTrack::flat_type *trackCollection, unsigned count,
                         int oneIn = 1);

double analyseMuons(TrackAll &trackCollection, unsigned count, int oneIn = 1);

double analyseMuons_v(TrackAll &trackCollection, unsigned count, int oneIn = 1);
