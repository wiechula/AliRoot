...
AliESDVertex* AliVertexerTracks::FindPrimaryVertex(const AliVEvent *vEvent)
{
    ...
    if(fClusterize){
        cout<<"1"<<endl;
        FindAllVertices(nTrksOrig,&trkArrayOrig,zTr,err2zTr,idOrig);
    }
    else
    {
        cout<<"2"<<endl;
...
//    if(!inputAOD) AnalyzePileUp((AliESDEvent*)vEvent);
    ...
    cout<<"AA:"<<fMVVertices->GetEntries()<<endl;
                                                   ...
                                                   UShort_t *idOrig)
{
    ...
    cout<<"veeertices:"<<fMVVertices->GetEntries()<<endl;
    ...
    cout<<"BB:"<<fMVVertices->GetEntries()<<endl;
    ...
    //
    cout<<"CC:"<<fMVVertices->GetEntries()<<endl;
    return fCurrentVertex;
}
//------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void AliVertexerTracks::VertexFitterNew(Bool_t vfit, Bool_t chiCalc,Int_t useWeights)
{
    Int_t fNTracks = (Int_t)fTrkArraySel.GetEntries();
    int fMaxVtxIter = 5;
    double fStopScaleChange = 0.8;
    double fBz = 5.0;
    ...
    fVert.SetNContributors(0); // invalidate
    if (fNTracks<3) return;
    fVert.SetXv(fNominalPos[0]);
    fVert.SetYv(fNominalPos[1]);
    fVert.SetZv(fNominalPos[2]);
    //
    double vtxXYZ[3] = {fNominalPos[0],fNominalPos[1],fNominalPos[2]};
    ...
    while(nIter++<fMaxVtxIter) {
        ...
        for (int itr=fNTracks;itr--;) {
            //
            AliExternalTrackParam& trc = *((AliExternalTrackParam*)fTrkArraySel.At(itr));
            ...
            if (TMath::Abs(detI)<kAlmost0) return;
            detI = 1./detI;
    ...
    for (int itr=fNTracks;itr--;) {
        ...
        trT.PropagateToDCA(&fVert,fBz,10,dz,covdum);
        ...
        //
        trS.PropagateToDCA(&fVert,fBz,10,dz,covdum);
        covt = (double*)trT.GetCovariance();
...
Bool_t AliVertexerTracks::FindNextVertexMV()
{
    ...
    do {
        ...
        if(newMethod)
        {
            VertexFitterNew(kTRUE,kFALSE,1);
        }
        else
        {
    ...
    else
    {
        ...
        if(newMethod)
        {
            VertexFitterNew(kFALSE,kTRUE,fMVFinalWBinary ? 2:1); // final chi2 calculation
        }
        else
        {
        ...
        for (int itr=0;itr<ntrk;itr++) {
            ...
            if (t->TestBit(kBitAccounted) || !t->TestBit(kBitUsed))
            {
                cout<<"already belong to a vertex"<<endl;
                continue;   // already belongs to some vertex
            }
        ...
        cout<<"mvvertices:"<<fMVVertices->GetEntries()<<endl;
