...
public:
  ...
  void    SetL1V0(Int_t i, const Int_t* v) {
    if (i) {
      for (int j = 0; j < 2; j++) fL1DCALV0[j] = v[j];
    } else {
      for (int j = 0; j < 2; j++) fL1V0[j] = v[j];
    }
  }
  ...
  /**
   * @brief Access to L0-amplitude (max amplitude) of the current fastor channel
   ...
   */
  Int_t   GetMedian(Int_t detectorIndex                       ) const {return (fMedian[detectorIndex] & 0x3FFFF);}
