...
AliAnalysisManager::AliAnalysisManager(const char *name, const char *title)
    : TNamed(name, title), fTree(0), fInputEventHandler(0),
      fOutputEventHandler(0), fMCtruthEventHandler(0), fEventPool(0),
      fCurrentEntry(-1), fNSysInfo(0), fMode(kLocalAnalysis), fInitOK(kFALSE),
      fMustClean(kFALSE), fIsRemote(kFALSE), fLocked(kFALSE), fMCLoop(kFALSE),
      fDebug(0), fSpecialOutputLocation(""), fTasks(0), fTopTasks(0),
      fZombies(0), fContainers(0), fInputs(0), fOutputs(0), fParamCont(0),
      fExchangeCont(0), fDebugOptions(0), fFileDescriptors(new TObjArray()),
      fCurrentDescriptor(0), fCommonInput(0), fCommonOutput(0), fSelector(0),
      fGridHandler(0), fExtraFiles(), fFileInfoLog(),
      fAutoBranchHandling(kTRUE),
      fAsyncReading(kFALSE), // default prefetching on
      fTable(), fRunFromPath(0), fNcalls(0), fMaxEntries(0),
      fCacheSize(100000000), // default 100 MB
      fStatisticsMsg(), fRequestedBranches(), fStatistics(0), fGlobals(0),
      fIOTimer(0), fCPUTimer(0), fInitTimer(0), fIOTime(0), fCPUTime(0),
      fInitTime(0) {
  // Default constructor.
...
AliAnalysisManager::AliAnalysisManager(const AliAnalysisManager &other)
    : TNamed(other), fTree(NULL), fInputEventHandler(NULL),
      fOutputEventHandler(NULL), fMCtruthEventHandler(NULL), fEventPool(NULL),
      fCurrentEntry(-1), fNSysInfo(0), fMode(other.fMode),
      fInitOK(other.fInitOK), fMustClean(other.fMustClean),
      fIsRemote(other.fIsRemote), fLocked(other.fLocked),
      fMCLoop(other.fMCLoop), fDebug(other.fDebug), fSpecialOutputLocation(""),
      fTasks(NULL), fTopTasks(NULL), fZombies(NULL), fContainers(NULL),
      fInputs(NULL), fOutputs(NULL), fParamCont(NULL), fExchangeCont(NULL),
      fDebugOptions(NULL), fFileDescriptors(new TObjArray()),
      fCurrentDescriptor(0), fCommonInput(NULL), fCommonOutput(NULL),
      fSelector(NULL), fGridHandler(NULL), fExtraFiles(other.fExtraFiles),
      fFileInfoLog(other.fFileInfoLog),
      fAutoBranchHandling(other.fAutoBranchHandling),
      fAsyncReading(other.fAsyncReading), fTable(), fRunFromPath(0),
      fNcalls(other.fNcalls), fMaxEntries(other.fMaxEntries),
      fCacheSize(other.fCacheSize), fStatisticsMsg(other.fStatisticsMsg),
      fRequestedBranches(other.fRequestedBranches),
      fStatistics(other.fStatistics), fGlobals(other.fGlobals),
      fIOTimer(new TStopwatch()), fCPUTimer(new TStopwatch()),
      fInitTimer(new TStopwatch()), fIOTime(0), fCPUTime(0), fInitTime(0) {
  // Copy constructor.
...
operator=(const AliAnalysisManager &other) {
  // Assignment
  if (&other != this) {
...
AliAnalysisManager::~AliAnalysisManager() {
  ...
  delete fEventPool;
  if (fgAnalysisManager == this)
    fgAnalysisManager = NULL;
  if (fGlobals) {
    fGlobals->DeleteAll();
    delete fGlobals;
  }
  if (fFileDescriptors) {
    fFileDescriptors->Delete();
    delete fFileDescriptors;
  }
  delete fIOTimer;
  delete fCPUTimer;
  delete fInitTimer;
...
void AliAnalysisManager::CreateReadCache() {
  ...
  if (!fCacheSize) {
    if (fDebug)
      Info("CreateReadCache", "=== Read caching disabled ===");
    return;
  }
  gEnv->SetValue("TFile.AsyncPrefetching", (Int_t)fAsyncReading);
  //   if (fAsyncReading)
  //   gEnv->SetValue("Cache.Directory",Form("file://%s/cache",
  ...
  if (!fAutoBranchHandling && !fRequestedBranches.IsNull()) {
    ...
    TObject *obj;
    while ((obj = next()))
      fTree->AddBranchToCache(obj->GetName(),
                              kTRUE); //<<< add requested branches to cache
    delete arr;
  } else {
    fTree->AddBranchToCache("*", kTRUE); //<<< add all branches to cache
  }
  if (fDebug) {
    Info("CreateReadCache",
         "Read cache enabled %lld bytes with async reading=%d", fCacheSize,
         (Int_t)fAsyncReading);
  }
  return;
}
...
Bool_t AliAnalysisManager::EventLoop(Long64_t nevents) {
  ...
  TTree *tree =
      new TTree("DummyTree", "Dummy tree for AliAnalysisManager::EventLoop");
...
Int_t AliAnalysisManager::GetRunFromAlienPath(const char *path) {
  ...
  // ALICE data in AliEn. It extracts the first number between slashes (/)
  // greater
  // than or equal to 18000 that appears after the /LHC.../ directory. This
  // covers
  ...
  while (spath.Tokenize(tok, from, "/")) {
    //::Info("AliAnalysisManager::GetRunFromAlienPath", "Token: {%s}",
    // tok.Data());
  ...
  if (expect == 4) {
    ...
    if (isData)
      ...
      type = "simulated";
    ::Info("AliAnalysisManager::GetRunFromAlienPath", "Run %d of %s data", run,
           type);
  } else {
    ::Error("AliAnalysisManager::GetRunFromAlienPath",
...
Bool_t AliAnalysisManager::Init(TTree *tree) {
  ...
  Bool_t init = kFALSE;
  if (!tree)
    return kFALSE; // Should not happen - protected in selector caller
  if (fDebug > 1) {
  ...
  if (!top)
    top = (AliAnalysisDataContainer *)fInputs->At(0);
  if (!top) {
...
void AliAnalysisManager::SlaveBegin(TTree *tree) {
  ...
  if (fDebug > 1)
    printf("->AliAnalysisManager::SlaveBegin()\n");
  // Init timer should be already started
  // Apply debug options
  ApplyDebugOptions();
  if (fCacheSize && fMCtruthEventHandler && (fMode != kProofAnalysis))
    fMCtruthEventHandler->SetCacheSize(fCacheSize);
  if (!CheckTasks())
    Fatal("SlaveBegin", "Not all needed libraries were loaded");
  ...
  if (isCalled && fMode == kMixingAnalysis)
    return;
  gROOT->cd();
  ...
  if (fOutputEventHandler) {
    if (fMode == kProofAnalysis) {
      ...
      if (fDebug > 1)
        printf("   Initializing AOD output file %s...\n",
               fOutputEventHandler->GetOutputFileName());
      init = fOutputEventHandler->Init("proof");
      if (!init)
        msg = "Failed to initialize output handler on worker";
    } else {
      init = fOutputEventHandler->Init("local");
      if (!init)
        msg = "Failed to initialize output handler";
    }
    initOK &= init;
    if (!fSelector)
      Error("SlaveBegin", "Selector not set");
    else if (!init) {
      fSelector->Abort(msg);
      fSelector->SetStatus(-1);
    }
  }
  gROOT->cd();
  if (fInputEventHandler) {
    ...
    if (fMode == kProofAnalysis) {
      ...
      if (!init)
        msg = "Failed to initialize input handler on worker";
    } else {
      init = fInputEventHandler->Init("local");
      if (!init)
        msg = "Failed to initialize input handler";
    }
    initOK &= init;
    if (!fSelector)
      Error("SlaveBegin", "Selector not set");
  ...
  if (fMCtruthEventHandler) {
    if (fMode == kProofAnalysis) {
      ...
      if (!init)
        msg = "Failed to initialize MC handler on worker";
    } else {
      init = fMCtruthEventHandler->Init("local");
      if (!init)
        msg = "Failed to initialize MC handler";
    }
    initOK &= init;
    if (!fSelector)
      Error("SlaveBegin", "Selector not set");
  ...
  while ((task = (AliAnalysisTask *)next())) {
    gROOT->cd();
    // Start with memory as current dir and make sure by default histograms do
    // not get attached to files.
    ...
    if (getsysInfo)
      AliSysInfo::AddStamp(Form("%s_CREATEOUTOBJ", task->ClassName()), 0, itask,
                           0);
    itask++;
  }
...
Bool_t AliAnalysisManager::Notify() {
  ...
  if (fDebug > 1)
    printf("<-AliAnalysisManager::Notify()\n");
  fIOTimer->Stop();
  fIOTime += fIOTimer->RealTime();
  return kTRUE;
}
...
void AliAnalysisManager::PackOutput(TList *target) {
  // Pack all output data containers in the output list. Called at
  // SlaveTerminate
  // stage in PROOF case for each slave.
  if (fDebug > 1)
    printf("->AliAnalysisManager::PackOutput()\n");
  fIOTimer->Start(kTRUE);
  std::ofstream out;
  if (IsCollectThroughput()) {
    if (fCurrentDescriptor)
    ...
    if (fFileInfoLog.IsNull())
      fFileInfoLog = "fileinfo.log";
    out.open(fFileInfoLog, std::ios::app);
    if (out.bad())
      Error("SavePrimitive", "Bad file name: %s", fFileInfoLog.Data());
    else {
      TIter nextflog(fFileDescriptors);
      TObject *log;
      while ((log = nextflog()))
        log->SavePrimitive(out, "");
    }
  }
  if (!target) {
  ...
  gROOT->cd();
  if (fInputEventHandler)
    fInputEventHandler->Terminate();
  if (fOutputEventHandler)
    fOutputEventHandler->Terminate();
  if (fMCtruthEventHandler)
    fMCtruthEventHandler->Terminate();
  gROOT->cd();
  ...
  while ((task = (AliAnalysisTask *)nexttask())) {
    if (!task->IsPostEventLoop()) {
      if (fDebug > 1)
        printf("->FinishTaskOutput: task %s\n", task->GetName());
  ...
  if (fMode == kProofAnalysis) {
    ...
    while ((output = (AliAnalysisDataContainer *)next())) {
      ...
      if (output->GetProducer() && output->GetProducer()->IsPostEventLoop())
        continue;
      ...
      if (!output->GetData() && !isManagedByHandler) {
        Error("PackOutput",
              "No data for output container %s. Forgot to PostData ?",
              output->GetName());
        continue;
      }
      if (!output->IsSpecialOutput()) {
        ...
        if (strlen(filename) && !isManagedByHandler) {
          ...
          if (!gSystem->AccessPathName(output->GetFileName()) && !firsttime)
            openoption = "UPDATE";
          //               TFile *file = AliAnalysisManager::OpenFile(output,
          //               openoption, kTRUE);
          ...
          if (output->GetData()->InheritsFrom(TCollection::Class())) {
            ...
            //                  coll->Write(output->GetName(),
            //                  TObject::kSingleKey);
          } else {
            if (output->GetData()->InheritsFrom(TTree::Class())) {
              TFile *file =
                  AliAnalysisManager::OpenFile(output, openoption, kTRUE);
              // Save data to file, then close.
              TTree *tree = (TTree *)output->GetData();
              // Check if tree is in memory
              if (tree->GetDirectory() == gROOT)
          ...
          if (fDebug > 1)
            printf("PackOutput %s: memory merge, file resident output\n",
                   output->GetName());
          //               if (fDebug > 2) {
          ...
          if (fDebug > 1)
            printf("PackOutput %s: memory merge memory resident output\n",
                   filename);
        }
        ...
        if (isManagedByHandler) {
          ...
          wrap = output->ExportData();
        // Output wrappers must NOT delete data after merging - the user owns
        // them
        wrap->SetDeleteData(kFALSE);
        target->Add(wrap);
      } else {
        // Special outputs. The file must be opened and connected to the
        // container.
        TDirectory *opwd = gDirectory;
        TFile *file = output->GetFile();
        if (!file) {
          AliAnalysisTask *producer = output->GetProducer();
          Fatal("PackOutput", "File %s for special container %s was NOT opened "
                              "in %s::CreateOutputObjects !!!",
                output->GetFileName(), output->GetName(),
                producer->ClassName());
          continue;
        }
        TString outFilename = file->GetName();
        if (fDebug > 1)
          printf("PackOutput %s: special output\n", output->GetName());
        if (isManagedByHandler) {
        ...
        if (opwd)
          opwd->cd();
        // Check if a special output location was provided or the output files
        // have to be merged
        if (strlen(fSpecialOutputLocation.Data())) {
          ...
          // No special location specified-> use TProofOutputFile as merging
          // utility
          // The file at this output slot must be opened in CreateOutputObjects
          if (fDebug > 1)
            printf("   File for container %s to be merged via file merger...\n",
                   output->GetName());
        }
      }
    }
  ...
  if ((fDebug || IsCollectThroughput())) {
    ...
    printf("=Analysis %s= init time:       %g[sec]\
           ...
           GetName(), fInitTime, fIOTime, fCPUTime, fInitTimer->CpuTime(),
           fInitTimer->RealTime());
    if (IsCollectThroughput()) {
      out << "#summary#########################################################"
          << endl;
  ...
  if (cdir)
    cdir->cd();
  if (fDebug > 1)
    printf("<-AliAnalysisManager::PackOutput: output list contains %d "
           "containers\n",
           target->GetSize());
}
...
void AliAnalysisManager::ImportWrappers(TList *source) {
  ...
  if (fDebug > 1)
    printf("->AliAnalysisManager::ImportWrappers()\n");
  fIOTimer->Start(kTRUE);
  TIter next(fOutputs);
  ...
  while ((cont = (AliAnalysisDataContainer *)next())) {
    wrap = 0;
    if (cont->GetProducer() && cont->GetProducer()->IsPostEventLoop() &&
        !inGrid)
      continue;
    ...
    if (cont->IsSpecialOutput() || inGrid) {
      ...
      if (!inGrid) {
        if (isManagedByHandler && fExtraFiles.Length()) {
          ...
          TIter nextfilename(arr);
          while ((os = (TObjString *)nextfilename()))
            GetFileFromWrapper(os->GetString(), source);
          delete arr;
        }
        if (!GetFileFromWrapper(filename, source))
          continue;
      }
      ...
      if (!f) {
        Error("ImportWrappers", "Cannot open file %s in read-only mode",
              filename);
        continue;
      }
      f->cd();
      ...
      if (!obj) {
        Warning("ImportWrappers", "Could not import object of type:%s for "
                                  "container %s in file %s:%s.\n Object will "
                                  "not be available in Terminate(). Try if "
                                  "possible to name the output object as the "
                                  "container (%s) or to embed it in a TList",
                cont->GetType()->GetName(), cont->GetName(), filename,
                cont->GetFolderName(), cont->GetName());
        continue;
      }
    ...
    if (!wrap) {
      Error("ImportWrappers", "Container %s not found in analysis output !",
            cont->GetName());
      continue;
    }
    icont++;
  ...
  fIOTime += fIOTimer->RealTime();
  if (fDebug > 1)
    printf("<-AliAnalysisManager::ImportWrappers(): %d containers imported\n",
           icont);
}
...
void AliAnalysisManager::UnpackOutput(TList *source) {
  ...
  fIOTimer->Start(kTRUE);
  if (fDebug > 1)
    printf("->AliAnalysisManager::UnpackOutput()\n");
  if (!source) {
    ...
    if (output->HasConsumers()) {
      ...
      for (Int_t i = 0; i < ncons; i++) {
        ...
        if (task->IsPostEventLoop() && task->IsActive()) {
          if (fDebug > 1)
            printf("== Executing post event loop task %s\n", task->GetName());
          if (fStatistics)
...
void AliAnalysisManager::Terminate() {
  ...
  while (!IsSkipTerminate() && (task = (AliAnalysisTask *)next())) {
    ...
    gROOT->cd();
    if (getsysInfo)
      AliSysInfo::AddStamp(Form("%s_TERMINATE", task->ClassName()), 0, itask,
                           2);
    itask++;
    if (TObject::TestBit(kSaveCanvases)) {
      if (!gROOT->IsBatch()) {
        if (fDebug > 1)
          printf("Waiting 5 sec for %s::Terminate() to finish drawing ...\n",
                 task->ClassName());
        timer.Start(kTRUE);
  ...
  //
  if (fInputEventHandler)
    fInputEventHandler->TerminateIO();
  if (fOutputEventHandler)
    fOutputEventHandler->TerminateIO();
  if (fMCtruthEventHandler)
    fMCtruthEventHandler->TerminateIO();
  gROOT->cd();
  TObjArray *allOutputs = new TObjArray();
  Int_t icont;
  for (icont = 0; icont < fOutputs->GetEntriesFast(); icont++)
    allOutputs->Add(fOutputs->At(icont));
  if (!IsSkipTerminate())
    for (icont = 0; icont < fParamCont->GetEntriesFast(); icont++)
      allOutputs->Add(fParamCont->At(icont));
  ...
  while ((output = (AliAnalysisDataContainer *)next1())) {
    ...
    icont++;
    if (fMode == kGridAnalysis && icont <= fOutputs->GetEntriesFast())
      continue;
    if (fMode == kProofAnalysis) {
      if (output->IsSpecialOutput() || output->IsRegisterDataset())
        continue;
    }
    const char *filename = output->GetFileName();
    TString openoption = "RECREATE";
    ...
    if (!file) {
      // if (handlerFile == filename && !gSystem->AccessPathName(filename))
      // openoption = "UPDATE";
      Bool_t firsttime = kTRUE;
      ...
      if (!gSystem->AccessPathName(filename) && !firsttime)
        openoption = "UPDATE";
      if (fDebug > 1)
        printf("Opening file: %s  option=%s\n", filename, openoption.Data());
      file = new TFile(filename, openoption);
    } else {
      if (fDebug > 1)
        printf("File <%s> already opened with option: <%s> \n", filename,
               file->GetOption());
      openoption = file->GetOption();
      if (openoption == "READ") {
        if (fDebug > 1)
          printf("...reopening in UPDATE mode\n");
        file->ReOpen("UPDATE");
      }
    }
    if (file->IsZombie()) {
    ...
    if (fDebug > 1)
      printf("...writing container %s to file %s:%s\n", output->GetName(),
             file->GetName(), output->GetFolderName());
    if (output->GetData()->InheritsFrom(TCollection::Class())) {
      // If data is a collection, we set the name of the collection
      // as the one of the container and we save as a single key.
      TCollection *coll = (TCollection *)output->GetData();
  ...
  while ((output = (AliAnalysisDataContainer *)next1())) {
    ...
    if (output->GetFile()) {
      ...
      if (fSpecialOutputLocation.BeginsWith("alien://")) {
        ...
        Info("Terminate", "Copy file %s to %s", output->GetFile()->GetName(),
             fSpecialOutputLocation.Data());
        gROOT->ProcessLine("if (!gGrid) TGrid::Connect(\"alien:\");");
        TFile::Cp(output->GetFile()->GetName(),
                  Form("%s/%s", fSpecialOutputLocation.Data(),
    ...
    if (!f.IsZombie()) {
      ...
      if (!gROOT->IsBatch()) {
        ...
        Int_t npads = 1 /*COO plot for all tasks*/ +
                      ...
                      1 /*Terminate plot for all tasks*/ + 1; /*vm plot*/
...
void AliAnalysisManager::ProfileTask(Int_t itop, const char *option) const {
  // Profiles the task having the itop index in the list of top (first level)
  // tasks.
  AliAnalysisTask *task = (AliAnalysisTask *)fTopTasks->At(itop);
  if (!task) {
    Error("ProfileTask", "There are only %d top tasks in the manager",
...
void AliAnalysisManager::ProfileTask(const char *name,
  ...
  if (fDebug > 1)
    printf("=== Profiling task %s (class %s)\n", name, task->ClassName());
  TCanvas *canvas = new TCanvas(
      Form("profile_%d", itop),
      Form("Profile of task %s (class %s)", name, task->ClassName()), 10, 10,
      800, 600);
...
void AliAnalysisManager::AddTask(AliAnalysisTask *task) {
  ...
  if (fInitOK) {
    Error("AddTask", "Cannot add task %s since InitAnalysis was already called",
          task->GetName());
    return;
  }

  if (fTasks->FindObject(task)) {
    Warning("AddTask", "Task %s: the same object already added to the analysis "
                       "manager. Not adding.",
            task->GetName());
    return;
  }
...
AliAnalysisManager::CreateContainer(const char *name, TClass *datatype,
                                    EAliAnalysisContType type,
                                    const char *filename) {
  ...
  // filename: composed by file#folder (e.g. results.root#INCLUSIVE) - will
  // write
...
Bool_t AliAnalysisManager::ConnectInput(AliAnalysisTask *task, Int_t islot,
  ...
  if (!fTasks->FindObject(task)) {
    AddTask(task);
    Info("ConnectInput",
         "Task %s was not registered. Now owned by analysis manager",
         task->GetName());
  }
...
Bool_t AliAnalysisManager::ConnectOutput(AliAnalysisTask *task, Int_t islot,
  ...
  if (!fTasks->FindObject(task)) {
    AddTask(task);
    Warning("ConnectOutput",
            "Task %s not registered. Now owned by analysis manager",
            task->GetName());
  }
...
void AliAnalysisManager::CleanContainers() {
  ...
  AliAnalysisDataContainer *cont;
  while ((cont = (AliAnalysisDataContainer *)next())) {
    if (cont->IsOwnedData() && cont->IsDataReady() && cont->ClientsExecuted())
      cont->DeleteData();
  }
}
...
Bool_t AliAnalysisManager::InitAnalysis() {
  ...
  while ((task = (AliAnalysisTask *)next())) {
    ...
    for (i = 0; i < ninputs; i++) {
      ...
      if (!cont) {
        ...
        Error("InitAnalysis", "Input slot %d of task %s has no container "
                              "connected ! Declared zombie...",
              i, task->GetName());
      }
      if (iszombie)
        continue;
      // Check if cont is an input container
      if (istop && !fInputs->FindObject(cont))
        istop = kFALSE;
  ...
  if (!ntop) {
    Error("InitAnalysis", "No top task defined. At least one task should be "
                          "connected only to input containers");
    return kFALSE;
  }
  ...
  // Check that all containers feeding post-event loop tasks are in the outputs
  // list
  ...
  while ((cont = (AliAnalysisDataContainer *)nextout())) {
    if (cont->IsSpecialOutput() && !strlen(cont->GetFileName())) {
      Error("InitAnalysis", "Wrong container %s : a file name MUST be provided "
                            "for special outputs",
            cont->GetName());
      return kFALSE;
    }
  ...
  if (!fAutoBranchHandling) {
    ...
    while ((task = (AliAnalysisTask *)next())) {
      if (!task->HasBranches()) {
        Error("InitAnalysis", "Manual branch loading requested but task %s of "
                              "type %s does not define branches.\nUse: "
                              "fBranchNames = \"ESD:br1,br2,...,brN "
                              "AOD:bra1,bra2,...,braM\"",
              task->GetName(), task->ClassName());
        return kFALSE;
      }
      if (!fInputEventHandler || !strlen(fInputEventHandler->GetDataType())) {
        Error("InitAnalysis", "Manual branch loading requested but no input "
                              "handler defined or handler does not define data "
                              "type.");
        return kFALSE;
      }
      ...
      if (taskbranches.IsNull()) {
        Error("InitAnalysis", "Manual branch loading requested but task %s of "
                              "type %s does not define branches of type %s:",
              task->GetName(), task->ClassName(),
              fInputEventHandler->GetDataType());
        return kFALSE;
      }
...
void AliAnalysisManager::CheckBranches(Bool_t load) {
  ...
  if (fAutoBranchHandling || fRequestedBranches.IsNull() || !fTree)
    return;
  TObjArray *arr = fRequestedBranches.Tokenize(",");
  TIter next(arr);
  TObject *obj;
...
Bool_t AliAnalysisManager::CheckTasks() const {
  ...
  if (!ntasks) {
    Error("CheckTasks", "No tasks connected to the manager. This may be due to "
                        "forgetting to compile the task or to load their "
                        "library.");
    return kFALSE;
  }
...
void AliAnalysisManager::PrintStatus(Option_t *option) const {
  ...
  if (!fInitOK) {
    Info("PrintStatus",
         "Analysis manager %s not initialized : call InitAnalysis() first",
         GetName());
    return;
  }
  Bool_t getsysInfo =
  ...
  if (getsysInfo) {
    AliSysInfo::SetVerbose(kTRUE);
    Info("PrintStatus", "System information will be collected each %lld events",
         fNSysInfo);
  }
...
//______________________________________________________________________________
Long64_t AliAnalysisManager::StartAnalysis(const char *type, Long64_t nentries,
                                           Long64_t firstentry) {
  // Start analysis having a grid handler.
  if (!fGridHandler) {
    Error("StartAnalysis", "Cannot start analysis providing just the analysis "
                           "type without a grid handler.");
    Info("===", "Add an AliAnalysisAlien object as plugin for this manager and "
                "configure it.");
    return -1;
  }
  TTree *tree = NULL;
...
Long64_t AliAnalysisManager::StartAnalysis(const char *type, TTree *const tree,
  ...
  if (!CheckTasks())
    Fatal("StartAnalysis", "Not all needed libraries were loaded");
  if (fDebug > 1) {
  ...
  else if (anaType.Contains("grid"))
    fMode = kGridAnalysis;
  else if (anaType.Contains("mix"))
    fMode = kMixingAnalysis;
  if (fInputEventHandler) {
    TString fname;
    InputFileFromTree(tree, fname);
    if (fname.Length()) {
      printf("Setting input file to: %s\n", fname.Data());
      fInputEventHandler->SetInputFileName(fname);
    }
  }
  ...
  if (fMode == kGridAnalysis) {
    ...
    if (!anaType.Contains("terminate")) {
      if (!fGridHandler) {
        ...
        Info("===", "Add an AliAnalysisAlien object as plugin for this manager "
                    "and configure it.");
        if (cdir)
          cdir->cd();
        return -1;
      ...
      if (fSelector && fSelector->GetStatus() == -1) {
        if (cdir)
          cdir->cd();
        return -1;
      }
      if (fGridHandler->GetRunMode() == AliAnalysisGrid::kOffline) {
        if (cdir)
          cdir->cd();
        return 0;
      }
      cout << "===== MERGING OUTPUTS REGISTERED BY YOUR ANALYSIS JOB: "
           << GetName() << endl;
  ...
  if (tree || fGridHandler || fMode == kMixingAnalysis)
    SetEventLoop(kTRUE);
  ...
  case kLocalAnalysis:
    if (!tree && !fGridHandler) {
      ...
      while ((task = (AliAnalysisTask *)nextT())) {
        ...
        if (getsysInfo)
          AliSysInfo::AddStamp(Form("%s_CREATEOUTOBJ", task->ClassName()), 0,
                               itask, 0);
        gROOT->cd();
        itask++;
      }
      ...
      if (IsExternalLoop()) {
        Info("StartAnalysis",
             "Initialization done. Event loop is controlled externally.\
  ...
  case kProofAnalysis:
    ...
    if (fGridHandler) {
      return StartAnalysis(type, fGridHandler->GetProofDataSet(), nentries,
                           firstentry);
    }
    ...
    if (chain) {
      chain->SetProof();
      cout << "===== RUNNING PROOF ANALYSIS " << GetName() << " ON CHAIN "
           << chain->GetName() << endl;
...
Long64_t AliAnalysisManager::StartAnalysis(const char *type,
  ...
  anaType.ToLower();
  if (!anaType.Contains("proof")) {
    Error("StartAnalysis", "Cannot process datasets in %s mode. Try PROOF.",
          type);
    return -1;
  }
  fMode = kProofAnalysis;
  ...
  gROOT->ProcessLine(line);
  Long_t retv;
  line.Form("gProof->Process((const char *)%p, \"AliAnalysisSelector\", "
            "\"%s\", %lld, %lld);",
  ...
  dispDataset[100] = '\0';
  cout << "===== RUNNING PROOF ANALYSIS " << GetName() << " ON DATASET "
       << dispDataset << endl;
  delete[] dispDataset;
...
Long64_t AliAnalysisManager::StartAnalysis(const char *type,
  ...
  gROOT->ProcessLine(line);
  Long_t retv;
  line.Form("gProof->Process((TFileCollection *)%p, \"AliAnalysisSelector\", "
            "\"%s\", %lld, %lld);",
            dataset, proofProcessOpt.Data(), nentries, firstentry);
  char *dispDataset = new char[101];
...
TFile *AliAnalysisManager::OpenFile(AliAnalysisDataContainer *cont,
                                    const char *option, Bool_t ignoreProof) {
  // Opens according the option the file specified by cont->GetFileName() and
  // changes
  // current directory to cont->GetFolderName(). If the file was already opened,
  // it
  // checks if the option UPDATE was preserved. File open via TProofOutputFile
  // can
  ...
  TFile *f = NULL;
  if (filename.IsNull()) {
    ::Error("AliAnalysisManager::OpenFile",
            "No file name specified for container %s", cont->GetName());
    return NULL;
  }
  if (mgr->GetAnalysisType() == AliAnalysisManager::kProofAnalysis &&
      cont->IsSpecialOutput() && !ignoreProof)
    ...
    if (f) {
      ...
      if ((opt == "UPDATE") && (opt != f->GetOption()))
        ::Info("AliAnalysisManager::OpenFile",
               "File %s already opened in %s mode!", cont->GetFileName(),
               f->GetOption());
    } else {
  ...
  ::Fatal("AliAnalysisManager::OpenFile", "File %s could not be opened",
          filename.Data());
  cont->SetFile(NULL);
  return NULL;
}
...
TFile *AliAnalysisManager::OpenProofFile(AliAnalysisDataContainer *cont,
  ...
  if (cont == fCommonOutput) {
    if (fOutputEventHandler) {
      ...
      Fatal("OpenProofFile", "No output container. Exiting.");
  }
  TFile *f = NULL;
  if (fMode != kProofAnalysis || !fSelector) {
    Fatal("OpenProofFile", "Cannot open PROOF file %s: no PROOF or selector",
          filename.Data());
    return NULL;
  }
  if (fSpecialOutputLocation.Length()) {
    ...
    if (f) {
      ...
      opt.ToUpper();
      if ((opt == "UPDATE") && (opt != f->GetOption()))
        ::Info("OpenProofFile", "File %s already opened in %s mode!",
               cont->GetFileName(), f->GetOption());
    } else {
  ...
  if (pof) {
    ...
    if (!f) {
      Fatal("OpenProofFile",
            "Proof output file found but no file opened for %s",
            filename.Data());
      return NULL;
    }
    // Check if option "UPDATE" was preserved
    ...
    opt.ToUpper();
    if ((opt == "UPDATE") && (opt != f->GetOption()))
      Fatal("OpenProofFile", "File %s already opened, but not in UPDATE mode!",
            cont->GetFileName());
  } else {
    if (cont->IsRegisterDataset()) {
      ...
      if (fDebug > 1)
        printf("Booking dataset: %s\n", dsetName.Data());
      line.Form("TProofOutputFile *pf = new TProofOutputFile(\"%s\", \"DROV\", "
                "\"%s\");",
                filename.Data(), dsetName.Data());
    } else {
      if (fDebug > 1)
        printf("Booking TProofOutputFile: %s to be merged\n", filename.Data());
      line.Form("TProofOutputFile *pf = new TProofOutputFile(\"%s\");",
                filename.Data());
    }
    if (fDebug > 1)
      printf("=== %s\n", line.Data());
...
void AliAnalysisManager::ExecAnalysis(Option_t *option) {
  ...
  if (fDebug > 0)
    ...
    if (!fIsRemote && TObject::TestBit(kUseProgressBar))
      ProgressBar("Processing event", fNcalls,
  ...
  if (fTree) {
    ...
    if (!cont)
      cont = (AliAnalysisDataContainer *)fInputs->At(0);
    if (!cont) {
      Error("ExecAnalysis", "Cannot execute analysis in TSelector mode without "
                            "at least one top container");
      if (cdir)
        cdir->cd();
      return;
    }
    cont->SetData(fTree); // This set activity for all tasks reading only from
                          // the top container
    ...
    if (fOutputEventHandler)
      fOutputEventHandler->BeginEvent(entry);
    if (fMCtruthEventHandler)
...
void AliAnalysisManager::SetInputEventHandler(AliVEventHandler *const handler) {
  ...
  if (!fCommonInput)
    fCommonInput = CreateContainer("cAUTO_INPUT", TChain::Class(),
                                   AliAnalysisManager::kInputContainer);
}

//______________________________________________________________________________
void AliAnalysisManager::SetOutputEventHandler(
    AliVEventHandler *const handler) {
  // Set the input event handler and create a container for it.
  Changed();
  fOutputEventHandler = handler;
  if (!fCommonOutput)
    fCommonOutput =
        CreateContainer("cAUTO_OUTPUT", TTree::Class(),
...
void AliAnalysisManager::SetDebugLevel(UInt_t level) {
  // Set verbosity of the analysis manager. If the progress bar is used, the
  // call is ignored
...
void AliAnalysisManager::RegisterExtraFile(const char *fname) {
  ...
  // connected to any output container, so that the manager can properly
  // register,
  // retrieve or merge them when running in distributed mode. The file names are
  // separated by blancs. The method has to be called in
  // MyAnalysisTask::LocalInit().
  if (fExtraFiles.Contains(fname))
    return;
...
Bool_t AliAnalysisManager::GetFileFromWrapper(const char *filename,
                                              const TList *source) {
  // Copy a file from the location specified ina the wrapper with the same name
  ...
  if (!pof || !pof->InheritsFrom("TProofOutputFile")) {
    Error("GetFileFromWrapper",
          "TProofOutputFile object not found in output list for file %s",
          filename);
    return kFALSE;
  }
  gROOT->ProcessLine(Form("sprintf((char*)%p, \"%%s\", "
                          "((TProofOutputFile*)%p)->GetOutputFileName());",
                          fullPath, pof));
  gROOT->ProcessLine(
      Form("sprintf((char*)%p, \"%%s\", gProof->GetUrl());", chUrl));
  ...
  if (clientUrl.Contains("localhost")) {
    ...
    fullPath_str.ReplaceAll(":PORT",
                            Form(":%s", strobjPort->GetString().Data()));
    if (fDebug > 1)
      Info("GetFileFromWrapper", "Using tunnel from %s to %s",
           fullPath_str.Data(), filename);
    delete arrayPort;
    delete array;
  } else if (clientUrl.Contains("__lite__")) {
    // Special case for ProofLite environement - get file info and copy.
    gROOT->ProcessLine(
        Form("sprintf((char*)%p,\"%%s\",((TProofOutputFile*)%p)->GetDir());",
             tmp, pof));
    fullPath_str.Form("%s/%s", tmp, fullPath);
  }
  if (fDebug > 1)
    Info("GetFileFromWrapper", "Copying file %s from PROOF scratch space to %s",
         fullPath_str.Data(), filename);
...
Bool_t AliAnalysisManager::ValidateOutputFiles() const {
  ...
  while ((output = (AliAnalysisDataContainer *)next())) {
    ...
    if (file) {
      Warning("ValidateOutputs", "File %s was not closed. Closing.",
              filename.Data());
      // Clear file list to release object ownership to user.
      //         file->Clear();
      file->Close();
    ...
    if (!file || file->IsZombie() || file->TestBit(TFile::kRecovered)) {
      Error("ValidateOutputs", "Output file <%s> was not created or invalid",
            filename.Data());
      if (cdir)
        cdir->cd();
      return kFALSE;
...
//______________________________________________________________________________
void AliAnalysisManager::ProgressBar(const char *opname, Long64_t current,
                                     Long64_t size, TStopwatch *const watch,
                                     Bool_t last, Bool_t refresh) {
  ...
  if (time > 0.) {
    Int_t full = Int_t(ocurrent > 0 ? time * (float(osize) / ocurrent) + .5
                                    : 99 * 3600 + 59 * 60 + 59);
    ...
    fprintf(stderr, "[%6.2f %%]%c", percent, lastChar);
  if (refresh && oneoftwo)
    oname = nname;
  if (owatch)
    owatch->Continue();
...
void AliAnalysisManager::DoLoadBranch(const char *name) {
  ...
  if (br->GetReadEntry() == fCurrentEntry)
    return;
  Long64_t readbytes = br->GetEntry(GetCurrentEntry());
  if (readbytes < 0) {
    Error("DoLoadBranch", "Could not load entry %lld from branch %s",
...
//______________________________________________________________________________
void AliAnalysisManager::CountEvent(Int_t ninput, Int_t nprocessed,
                                    Int_t nfailed, Int_t naccepted) {
...
void AliAnalysisManager::AddStatisticsMsg(const char *line) {
  // Add a line in the statistics message. If available, the statistics message
  // is written
  // at the end of the SlaveTerminate phase on workers AND at the end of
  // Terminate
  // on the client.
  if (!strlen(line))
    return;
  if (!fStatisticsMsg.IsNull())
    fStatisticsMsg += "\n";
  fStatisticsMsg += line;
}
...
void AliAnalysisManager::WriteStatisticsMsg(Int_t) {
  // If fStatistics is present, write the file in the format
  // ninput_nprocessed_nfailed_naccepted.stat
  ...
  ofstream out;
  AddStatisticsMsg(
      Form("Number of input events:        %lld", fStatistics->GetNinput()));
  AddStatisticsMsg(Form("Number of processed events:    %lld",
                        fStatistics->GetNprocessed()));
  AddStatisticsMsg(
      Form("Number of failed events (I/O): %lld", fStatistics->GetNfailed()));
  AddStatisticsMsg(Form(
      "Number of accepted events for mask %s: %lld",
      AliAnalysisStatistics::GetMaskAsString(fStatistics->GetOfflineMask()),
      fStatistics->GetNaccepted()));
...
const char *AliAnalysisManager::GetOADBPath() {
  ...
  if (gSystem->Getenv("OADB_PATH"))
    ...
    ::Fatal("AliAnalysisManager::GetOADBPath",
...
void AliAnalysisManager::SetGlobalStr(const char *key, const char *value) {
  // Define a custom string variable mapped to a global unique name. The
  // variable
  ...
  if (valid) {
    ::Error("AliAnalysisManager::SetGlobalStr",
...
void AliAnalysisManager::SetGlobalInt(const char *key, Int_t value) {
  // Define a custom integer variable mapped to a global unique name. The
  // variable
  ...
  if (valid) {
    ::Error("AliAnalysisManager::SetGlobalInt",
            "Global %s = %i already defined.", key, existing);
    return;
  }
  mgr->GetGlobals()->Add(new TObjString(key),
                         new TObjString(TString::Format("%i", value)));
}
...
void AliAnalysisManager::SetGlobalDbl(const char *key, Double_t value) {
  // Define a custom double precision variable mapped to a global unique name.
  // The variable
  // can be then retrieved by a given analysis macro via GetGlobalInt(key).
  AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
  if (!mgr) {
  ...
  mgr->GetGlobals()->Add(new TObjString(key),
                         new TObjString(TString::Format("%f.16", value)));
}
...
void AliAnalysisManager::AddClassDebug(const char *className,
  ...
  if (!debugOpt) {
    AliInfo(TString::Format("Adding debug level %d for class %s", debugLevel,
                            className)
                .Data());
    fDebugOptions->Add(
        new TNamed(className, TString::Format("%d", debugLevel).Data()));
  } else {
    TString oldDebugStr = debugOpt->GetTitle();
    Int_t oldDebug = oldDebugStr.Atoi();
    if (debugLevel > oldDebug) {
      AliWarning(
          TString::Format("Overwriting debug level to %d class %s, because it "
                          "is higher then previously set (%d).",
                          debugLevel, className, oldDebug)
              .Data());
      debugOpt->SetTitle(TString::Format("%d", debugLevel).Data());
    } else {
      AliWarning(TString::Format("Ignoring debug level to %d class %s, because "
                                 "it is smaller then previously set (%d).",
                                 debugLevel, className, oldDebug)
                     .Data());
    }
  }
...
void AliAnalysisManager::ApplyDebugOptions() {
  ...
  while ((debug = dynamic_cast<TNamed *>(next()))) {
    ...
    AliInfo(TString::Format("Class=%s debulLevel=%d", debug->GetName(),
                            debugLevel.Atoi())
                .Data());
...
//______________________________________________________________________________
Int_t AliAnalysisManager::LoadMacro(const char *filename, Int_t *error,
                                    Bool_t check) {
  // Redirection of gROOT->LoadMacro which makes sure the same macro is not
  // loaded
  // twice
...
void AliAnalysisManager::Lock() {
  ...
  fLocked = kTRUE;
  if (fInputEventHandler)
    fInputEventHandler->Lock();
  if (fOutputEventHandler)
    fOutputEventHandler->Lock();
  if (fMCtruthEventHandler)
    fMCtruthEventHandler->Lock();
...
void AliAnalysisManager::UnLock() {
  ...
  fLocked = kFALSE;
  if (fInputEventHandler)
    fInputEventHandler->UnLock();
  if (fOutputEventHandler)
    fOutputEventHandler->UnLock();
  if (fMCtruthEventHandler)
    fMCtruthEventHandler->UnLock();
  Info("UnLock", "====== ANALYSIS MANAGER UNLOCKED ======");
}

//______________________________________________________________________________
void AliAnalysisManager::Changed() {
  // All critical setters pass through the Changed method that throws an
  // exception
  // in case the lock was set.
  if (fLocked)
    Fatal("Changed", "Critical setter called in locked mode");
}
