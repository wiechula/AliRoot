...
public:
  enum EAliAnalysisContType {
    ...
    kParamContainer =
        3 // use for storing task private parameters/cuts per analysis session
  };
  ...
  virtual Long64_t StartAnalysis(const char *type, TTree *const tree,
                                 Long64_t nentries = 1234567890,
                                 Long64_t firstentry = 0);
  virtual Long64_t StartAnalysis(const char *type, const char *dataset,
                                 Long64_t nentries = 1234567890,
                                 Long64_t firstentry = 0);
  virtual Long64_t StartAnalysis(const char *type, TFileCollection *dataset,
                                 Long64_t nentries = 1234567890,
                                 Long64_t firstentry = 0);
  virtual Long64_t StartAnalysis(const char *type,
                                 Long64_t nentries = 1234567890,
                                 Long64_t firstentry = 0);
  ...
  static Int_t LoadMacro(const char *filename, Int_t *error = 0,
                         Bool_t check = kFALSE);
  EAliAnalysisExecMode GetAnalysisType() const { return fMode; }
  ...
  Bool_t IsProofMode() const {
    return (fMode == kProofAnalysis) ? kTRUE : kFALSE;
  }
  ...
  void SetCollectThroughput(Bool_t flag) {
    Changed();
    TObject::SetBit(kCollectThroughput, flag);
  }
  ...
  void SetSaveCanvases(Bool_t flag = kTRUE) {
    TObject::SetBit(kSaveCanvases, flag);
  }
  ...
  CreateContainer(const char *name, TClass *datatype,
  ...
  void CountEvent(Int_t ninput, Int_t nprocessed, Int_t nfailed,
                  Int_t naccepted);
  Bool_t InitAnalysis();
  Bool_t IsInitialized() const { return fInitOK; }
  Bool_t IsTrainInitialized() const {
    return TObject::TestBit(kTasksInitialized);
  }
  ...
  static void ProgressBar(const char *opname, Long64_t current, Long64_t size,
                          TStopwatch *const watch = 0, Bool_t last = kFALSE,
...
protected:
  ...
  AliVEventHandler
      *fOutputEventHandler; //  Optional common output event handler
  AliVEventHandler
      *fMCtruthEventHandler; //  Optional common MC Truth event handler
  ...
  TString fSpecialOutputLocation; // URL/path where the special outputs will be
                                  // copied
  ...
  static AliAnalysisManager
      *fgAnalysisManager; //! static pointer to object instance
