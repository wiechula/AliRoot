 ...
 *$Id$
 *$Log$
 *Revision 1.6  2007/05/09 08:43:02  panos
...
#include "AliFemtoEventReaderESDWithFriend.h"
...
ClassImp(AliFemtoEventReaderESDWithFriend)
...
AliFemtoEventReaderESDWithFriend::AliFemtoEventReaderESDWithFriend():
  fInputFile(" "),
  fFileName(" "),
  fConstrained(true),
  fNumberofEvent(0),
...
AliFemtoEventReaderESDWithFriend::AliFemtoEventReaderESDWithFriend(const AliFemtoEventReaderESDWithFriend &aReader) :
  fInputFile(" "),
  fFileName(" "),
  fConstrained(true),
  fNumberofEvent(0),
...
//__________________
AliFemtoEventReaderESDWithFriend& AliFemtoEventReaderESDWithFriend::operator=(const AliFemtoEventReaderESDWithFriend& aReader)
{
  if (this == &aReader)
    return *this;
...
//__________________
AliFemtoString AliFemtoEventReaderESDWithFriend::Report()
{
  AliFemtoString temp = "\n This is the AliFemtoEventReaderESDWithFriend\n";
  return temp;
}
...
void AliFemtoEventReaderESDWithFriend::SetInputFile(const char* inputFile)
{
  char buffer[256];
...
bool AliFemtoEventReaderESDWithFriend::GetNextFile()
{ 	
...
void AliFemtoEventReaderESDWithFriend::SetConstrained(const bool constrained)
{
  fConstrained=constrained;
}

bool AliFemtoEventReaderESDWithFriend::GetConstrained() const
{
  return fConstrained;
}

AliFemtoEvent* AliFemtoEventReaderESDWithFriend::ReturnHbtEvent()
{
  ...
  if (fCurEvent==fNumberofEvent)//open next file  
      ...
      if(GetNextFile())	
	{
	  ...
	  fTree->SetBranchStatus("ESDfriend*",1);
	  fTree->SetBranchStatus("ESDfriend.fTracks.fCalibContainer",0);
	  ...
	  fEventFriend = (AliESDfriend*)fEvent->FindListObject("AliESDfriend");
	  if (!fEventFriend) {
...
//	  fTree->SetBranchStatus("fTracks.fCalibContainer",0);
  ...
  for (int i=0;i<nofTracks;i++)
	...
	if (tTrackIndices[tNcl] > 0) {
      ...
      // Copy cluster information from the ESD
