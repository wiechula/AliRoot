  ...
  fNtot(0)
{
  ...
  TFile *filepar = TFile::Open("$ALICE_ROOT/ZDC/fragOO.root");
  if (!filepar){
    printf(" No file $ALICE_ROOT/ZDC/fragOO.root found \n");
    return;
  }
  
  TF1 *fzb = (TF1*) filepar->Get("fzb");
  fZboundvsbOO = (TF1*) filepar->Get("fzb");
  filepar->Close();
...
void AliZDCFragment::GenerateIMF()
{
   ...
   fNimf = Int_t(0.5+averageNimf+fluctNimf);
...
//   printf("\n fZmax = %f\n", fZmax);
   ...
   fNalpha = Int_t(0.5+averageAlpha+fluctAlpha);
...
//   printf("\n choice = %d, fZbAverage = %f, zbFrag = %f \n", choice, fZbAverage, zbFrag);
   ...
   for(int j=0; j<fNimf; j++){
      zz[j] =0;
   }
   for(int i=0; i<fNimf; i++){
...
//   }
   ...
   for(int j=0; j<fNimf; j++){
     fZZ[j] = Int_t (0.5+zz[j]*fZmax/zz[fNimf-1]);
     if(fZZ[j]<3) fZZ[j] = 3;
//     printf("\n 	fZZ[%d] = %d \n",j,fZZ[j]);
   }
   ...
   if(sumZ>zbFrag){
     ...
     if(choice == 1) return;
     Int_t iDiff = Int_t(0.5+(zbFrag-sumZ)/2);
     if(iDiff<fNalpha){
       fNalpha=iDiff;
       return;
     }
...
//_____________________________________________________________________________
void AliZDCFragment::GenerateIMFOO()
{
   ...
   const Float_t  kParamFluctNimf[4]={-0.13176,2.9392,-5.2147,2.3092};
   // Coefficients of polynomial for average maximum Z of fragments
   const Float_t  kParamZmax[4]={0.0165,1.3856,-0.2694,6.74};
   // Coefficients of polynomial for fluctuations on maximum Z of fragments
   const Float_t  kParamFluctZmax[5]={0.013782/10.5,-0.17282/10.5,1.5065/10.5,1.0654/10.5,-2.4317/10.5};
   // Coefficients of polynomial for exponent tau of fragments Z distribution
   const Float_t  kParamTau[3]={6.7233,-15.85,13.047};
   //Coefficients of polynomial for average number of alphas
   const Float_t  kParamNalpha[4]={-0.68554/10.5,39.605/10.5,-68.311/10.5,30.165/10.5};
   // Coefficients of polynomial for fluctuations on average number of alphas
   const Float_t  kParamFluctNalpha[5]={0.283,6.2141,-17.113,17.394,-6.6084};
  
   // Maximum impact parameter for U [r0*A**(1/3)]
   const Float_t  kbMaxU = 14.87;
   // Maximum impact parameter for O from WS
   const Float_t  kbMaxO = 2*(2.7+0.2);//5.8 fm
   // Z of the projectile
   const Float_t  kZProj = 8.;
   
   fZbAverage = fZboundvsbOO->Eval(fB);
   //printf("   b %f   Zbound %f\n", fB, fZbAverage);
   ...
   // b=kbMaxPb-2kNuclearThick the two functions have the same derivative
   //Float_t bCore = kbMaxPb-2*kNuclearThick;
   //if(fB>kbMaxPb){ // non sara' mai a causa della def. a linea 347!!!
   //  fZbAverage = TMath::Exp(-kParamSkinPb[0]*(fB-kParamSkinPb[1]));
   //  //printf(" b = %1.2f fm   Z_bound %1.2f\n", fB, fZbAverage);
   //}
   if(fZbAverage>kZProj) fZbAverage = kZProj;
   Float_t zbNorm = fZbAverage/kZProj;
   Float_t bNorm = fB/kbMaxO;
...
//   printf("\n fZmax = %f\n", fZmax);
   ...
   if(bNorm<=0.9 && fZbAverage>=(fNalpha*2)) { // second condition added for O (zbFrag>0)!!!
     // remove alpha from zbound to find zbound for fragments  (Z>=3)
     zbFrag = fZbAverage-fNalpha*2;
     choice = 1;
   }
