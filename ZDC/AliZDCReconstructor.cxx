...
void AliZDCReconstructor::Reconstruct(AliRawReader* rawReader, TTree* clustersTree) const
{
  ...
  while (rawData.Next()) {
    if(rawData.IsADCDataWord()){
      if(rawData.GetADCGain() == 0){
	if(rawData.GetSector(0) == 1)	   
	  zn1corr  += (Float_t) (rawData.GetADCValue()-meanPed[rawData.GetSector(1)]); // high gain ZN1 ADCs;
	else if(rawData.GetSector(0) == 2) 
	  zp1corr  += (Float_t) (rawData.GetADCValue()-meanPed[rawData.GetSector(1)+10]); // high gain ZP1 ADCs;
	else if(rawData.GetSector(0) == 3){
	  if(rawData.GetSector(1)==1)      
	    zemcorr += (Float_t) (rawData.GetADCValue()-meanPed[rawData.GetSector(1)+20]); // high gain ZEM1 ADCs
	  else if(rawData.GetSector(1)==2) 
	    zemcorr += (Float_t) (rawData.GetADCValue()-meanPed[rawData.GetSector(1)+22]); // high gain ZEM2 ADCs
	}
	else if(rawData.GetSector(0) == 4)	   
	  zn2corr  += (Float_t) (rawData.GetADCValue()-meanPed[rawData.GetSector(1)+24]); // high gain ZN2 ADCs;
	else if(rawData.GetSector(0) == 5) 
	  zp2corr  += (Float_t) (rawData.GetADCValue()-meanPed[rawData.GetSector(1)+34]); // high gain ZP2 ADCs;
      }
    }
  }
  if(zn1corr<0) zn1corr=0;
