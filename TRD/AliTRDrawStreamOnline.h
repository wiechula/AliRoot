...
#ifndef ALITRDRAWSTREAMONLINE_H
#define ALITRDRAWSTREAMONLINE_H
...
class AliTRDrawStreamOnline : public AliTRDrawStreamBase
{
 public:
  AliTRDrawStreamOnline(AliRawReader *rawReader = 0x0);
  ~AliTRDrawStreamOnline();
  ...
  // event statistics
  class AliTRDrawStats{
  public:
    AliTRDrawStats() : fBytesRead(0) {}
    void ClearStats();

    class AliTRDrawStatsSector{
    public:
      AliTRDrawStatsSector() : fBytes(0), fBytesRead(0), fNTracklets(0), fNMCMs(0), fNChannels(0) {}
      void ClearStats();

      class AliTRDrawStatsHC{
      public:
        AliTRDrawStatsHC() : fBytes(0), fBytesRead(0), fNTracklets(0), fNMCMs(0), fNChannels(0) {}
	void ClearStats();
 ...
 protected:
  ...
  static const char *const fgErrorMessages[kLastErrorCode];     // error messages corresponding to the error codes
  static const Int_t fgErrorDebugLevel[kLastErrorCode];   // error debug level
  static const ErrorBehav_t fgErrorBehav[kLastErrorCode]; // bevhaviour in case of error of given type
  ...
  AliTRDrawStreamOnline(const AliTRDrawStreamOnline&);           // not implemented
  AliTRDrawStreamOnline& operator=(const AliTRDrawStreamOnline&); // not implemented

  ClassDef(AliTRDrawStreamOnline, 0);
};
