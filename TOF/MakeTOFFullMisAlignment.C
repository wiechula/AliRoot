void MakeTOFFullMisAlignment(){
  ...
  Info(macroname,"Loading FRAME alignment objects from CDB storage %s",
      Storage.Data());
  AliCDBPath fpath("FRAME","Align","Data");
  if( gSystem->Getenv("TOCDB") == TString("kTRUE") ){
  ...
  for(Int_t i=0; i<nSMTOF; i++) {
    ...
    dphi = 0.;
    new((*array)[j++]) AliAlignObjAngles(symname.Data(), dvoluid, smdx, smdy, smdz, dpsi, dtheta, dphi, kFALSE);
  }
  // Apply objects for TOF supermodules 
  for(Int_t i=0; i<nSMTOF; i++){
    AliAlignObjAngles* smobj = (AliAlignObjAngles*)array->UncheckedAt(i);
  ...
  for(i=0; i<AliGeomManager::LayerSize(idTOF); i++) {
    ...
    sdtheta = 0.;
    sdphi = 0.;
    new((*array)[j++]) AliAlignObjAngles(AliGeomManager::SymName(idTOF,i), AliGeomManager::LayerToVolUID(idTOF,i), sdx, sdy, sdz, sdpsi, sdtheta, sdphi, kFALSE);
  }
