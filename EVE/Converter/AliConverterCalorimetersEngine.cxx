...
)
{
    for (Int_t icell = 0; icell < fCaloCluster->GetNCells(); icell++){
        ...
        AliMinimalisticCaloCluster cluster(
                ...
                eta,
                TMath::Pi() / 80.,  // Uwazam, ze nalezy zapisac te wartosc do zmiennej, nie bardzo wiadomo skad sie bierze taka wartosc
                3./200.,          // Poza tym, przy kazdym wywolaniu podajemy te sama wartosc. Nie lepiej zapisac to jako element klasy
                amp               // Tak, aby było to na sztywno ustawione? Dla kogos, kto bedzie chciał to debugowac za x lat bedzie to skrajnie nieczytelne
        );
        event.AddCaloCluster(cluster);
    }
}
...
// W ponizszej metodzie jest bardzo duzo wartosci do, ktorych porownujemy - nie bardzo wiadomo co, ktora oznacz
