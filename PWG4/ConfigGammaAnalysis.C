...
//     reader->SetEMCALMinDistance(3.6);    reader->SetPHOSMinDistance(11.); //Miimum overlapp distance
    ...
    gd->SetConeSize(0.3); gd->SetPtThreshold(1.); gd->SetPtSumThreshold(1.);
    gd->SetICMethod(AliAnaGammaDirect::kNoIC) ;//Options:
          ...
          //kSeveralIC: Automatically charged for case kIsolationCut
//     //for option kIsolationCut:
...
//     //and many more setters and getters
    ...
    ana->SetAnalysisType(AliAnaGamma::kIsolationCut); //set kPrompt, kIsolationCut or kCorrelation
    //ana->SetGammaCorrelation(gc);//pointer to algorithm
