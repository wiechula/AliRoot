...
{ 
  ...
  const Int_t kFirstEvent = atoi(gSystem->Getenv("EVENT")) ;  
...
//"fEventTag.fNPHOSClustersMin == 1 && fEventTag.fNEMCALClustersMin == 1" ; 
  ...
  if ( input != "") {
	...
	return ;
      ...
      Int_t event, skipped, count = 0 ; 
      char file[120] ;
      Double_t xsection = 0., ntrials = 0. ; 
      Double_t * rv = new Double_t[2] ;
      rv[0] = rv[1] = 0.0 ; 
	for (event = kFirstEvent ; event < kFirstEvent+kEvent ; event++) {
        sprintf(file, "%s/%d/AliESDs.root", kInDir,event) ; 
	TFile * fESD = 0 ; 
	if ( fESD = TFile::Open(file)) { 
	  ReadXsection(kInDir, event, rv) ; 
	  if ( fESD->Get("esdTree") && rv ) { 
            printf("++++ Adding %s\n", file) ;
...
void ReadXsection(const char * inDir, const Int_t event, Double_t * rv)
{
  ...
  if ( ! file ) {
    AliError(Form("could not open %s", cfile)) ; 
    return 0x0 ; 
  }
