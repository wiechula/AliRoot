...
void anaPhos(const Int_t kEvent=10)  
{
  ...
  AliAnaGammaPrompt * gammaPromptPhos = new AliAnaGammaPrompt("GammaPromptPhos") ; 
  ag->ConnectInput(gammaPromptPhos, outPHOSGamma, 0) ; 
  ag->ConnectInput(gammaPromptPhos, outCharged,   1) ; 
  AliAnalysisDataContainer * outPHOSGammaPrompt  = ag->ConnectOuput(gammaPromptPhos, TClonesArray::Class(), 0) ;
  ag->ConnectOuput(gammaPromptPhos, TList::Class(), 1) ;

  AliAnaGammaPrompt * gammaPromptEmcal = new AliAnaGammaPrompt("GammaPromptEmcal") ; 
  ag->ConnectInput(gammaPromptEmcal, outEMCALGamma, 0) ; 
  ag->ConnectInput(gammaPromptEmcal, outCharged,    1) ; 
  AliAnalysisDataContainer * outEMCALGammaPrompt  = ag->ConnectOuput(gammaPromptEmcal, TClonesArray::Class(), 0) ;
  ag->ConnectOuput(gammaPromptEmcal, TList::Class(), 1) ;

  AliAnaGammaCorr * corrPhos = new AliAnaGammaCorr("CorrPartonPhos") ;
  corrPhos->SetAnalysis(AliAnaGammaCorr::kParton) ; // kJet, kHadron 
  ag->ConnectInput(corrPhos, outPHOSGammaPrompt, 0) ; 
  ag->ConnectInput(corrPhos, outParton, 1) ; 
  AliAnalysisDataContainer * outPartonPhos  = ag->ConnectOuput(corrPhos, TNtuple::Class(), 0) ; 

  AliAnaGammaCorr * corrEmcal = new AliAnaGammaCorr("CorrPartonEmcal") ; 
  corrEmcal->SetAnalysis(AliAnaGammaCorr::kParton) ;
  ag->ConnectInput(corrEmcal, outEMCALGammaPrompt, 0) ; 
  ag->ConnectInput(corrEmcal, outParton, 1) ; 
  AliAnalysisDataContainer * outPartonEmcal  =  ag->ConnectOuput(corrPartonEmcal, TNtuple::Class(), 0) ; 
